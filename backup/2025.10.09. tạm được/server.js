...
function computeDeltas(latest, prev) {
  ...
  const out = JSON.parse(JSON.stringify(latest)); // deep enough clone for our use
  ...
  brandKeys.forEach(brand => {
    ...
    // ensure sell change fields exist (leave as-is if present)
  ...
  // also compute silver deltas if structure present (optional)
...
// Backward-compatible route expected by the frontend client
app.get('/scrape', (req, res) => {
  ...
  const prev = prevFile ? loadSnapshot(prevFile) : null;
  const result = computeDeltas(latest, prev);
  // return result directly (same shape client expects in fetchRealUpdate)
