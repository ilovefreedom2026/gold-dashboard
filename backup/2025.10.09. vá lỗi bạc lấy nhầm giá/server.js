...
async function scrapeSilver() {
  try {
    ...
    $('tr').each((i, tr) => {
      ...
      // identify "thỏi" candidate: require THỎI or KILO/KG in unit
      const isTh = prodText.includes('THỎI') || /\b(KG|KILO)\b/.test(unitText) || rowText.includes('THỎI');

      if (isMi) {
        // pick numeric values from buyCells first if present
